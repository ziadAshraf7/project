...
export default function EditModal({
    ...
    if (addStatus) {
        return (
            <form onSubmit={async () => {
            ...
            }} ref={targetRef} className='absolute p-4 z-10 rounded-lg  h-[196px] flex flex-col items-end left-[50%] w-[294px] top-[50%] translate-x-[-50%] translate-y-[-50%] bg-[#33363F]'>
                ...
                <div className='mb-4'>
                    <input required className='w-[167px]' onChange={(e) => setNewAddedFloorName(e.target.value)} value={newFloorName} type='text' />
    ...
    return (
        <form onSubmit={async () => {
            ...
            <div className='mb-4'>
                <input required className='w-[167px]' onChange={(e) => setNewName(e.target.value)} value={newName} type='text' />
                <span className='text-white p-2'>{t("name")}</span>
            </div>

            <div className='mb-4'>
                <input required className='w-[167px]' onChange={(e) => setFloorName(e.target.value)} value={newFloorName} type='text' />
                <span className='text-white p-2'>{t("floor")}</span>
            </div>

            <div className=''>
                <input required className='w-[167px]' onChange={(e) => setCode(e.target.value)} value={newCode} type='text' />
